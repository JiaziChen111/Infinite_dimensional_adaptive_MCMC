function [model, samples, accRates] = CN_cmeasure_s(model, mcmcoptions)
%%
%
...
model.A = model.U * diag(sqrt(model.Lambda));
...
%     
    ...
    Enew =   sqrtOneBeta.*E + beta.*(D.*randn( model.n, 1));
    %
    if strcmp(model.adaptive,'eigen')
        Xnew=   Enew;%model.U * (sqrt(model.Lambda).* Enew);
    elseif strcmp(model.adaptive,'chol')
    ...
    %
        ...
        samples.X(cnt,:) = X;  
        samples.E(cnt,:) = E;
        samples.beta_vec(cnt) = model.beta;
    end
    %    
