function [model, samples, accRates] = LCN_cmeasure(model, mcmcoptions)
%%
% 
...
for it = 1:(BurnInIters + Iters) 
    ...
    Emean = (1-sqrtOneBeta2) * ( E+D2.*(derE - E));
    Enew =   Emean + sqrtOneBeta2.*E + beta*(D.*randn( model.n, 1));
     %
    if strcmp(model.adaptive,'eigen')
        Xnew=   U_L* Enew;
    ...
    Emean_new = (1-sqrtOneBeta2)* (Enew + (D2.*(derE_new- Enew))) ;
    
   
    
    % Metropolis-Hastings to accept-reject the proposal
    hxy = newLogLik + (E    - sqrtOneBeta2.*Enew - 0.5 * Emean_new)'*(Emean_new./(beta.^2*D2));
    hyx = oldLogLik + (Enew - sqrtOneBeta2.*E    - 0.5 * Emean)'*(Emean./(beta.^2*D2));
    sumQnew = 0.5*sum(Q.*Enew.^2);
    hxy = hxy  - sumQnew + sumQ;
